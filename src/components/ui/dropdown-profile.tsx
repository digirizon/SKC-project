...
const getInitials = (email: string): string => {
  if (!email) return "U"; // Default for User
  const emailPrefix = email.split("@")[0];
  if (!emailPrefix) return "U";

  const nameParts = emailPrefix.split(/[._-]/); // Split by common separators like '.', '_', '-'
  ...
  return emailPrefix.substring(0, 1).toUpperCase() || "U"; // Fallback to first letter or "U"
};
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="sm" className="flex items-center">
          <User className="w-5 h-5 md:hidden" /> {/* Show User icon on small screens */}
          ...
          <span className="ml-2 md:hidden">{userInitials}</span> {/* Show initials text on small screens if preferred */}
        </Button>
      </DropdownMenuTrigger>
